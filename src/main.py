...
def _residual_block11(block_function, filters, repetitions, is_first_layer=False):
    """Builds a residual block with repeating bottleneck blocks.
    """
    def f(input):
        for i in range(repetitions):
            ...
            if i == 0 and not is_first_layer:
                init_strides = (2, 2)
            input = block_function(filters=filters, init_strides=init_strides,
                                   is_first_block_of_first_layer=(False and i == 0))(input)
        return input
...
class ResnetBuilder(object):
    ...
    def build(input_shape, block_fn, repetitions):
        ...
        for i, r in enumerate(repetitions):
            ...
            filters *= 2
        FTGenerator3= _conv_bn_relu(filters=64, kernel_size=(3,3), strides =(1,1),name = 'FT3' )(block)
        FTGenerator2= _conv_bn_relu(filters=32, kernel_size=(3,3), strides =(1,1), name = 'FT2' )(FTGenerator3)
        FTGenerator1 = MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="same", name = 'FT1')(FTGenerator2)
        FTGenerator = _conv_bn_relu(filters=1, kernel_size=(3,3), strides =(1,1), name = 'FT' )(FTGenerator1)



        for i, r in enumerate(repetitions):
            block = _residual_block11(block_fn, filters=filters, repetitions=r, is_first_layer=False)(block)
            filters *= 2
        ...
        model = Model(inputs=input, outputs=[FTGenerator,D1_faceImg, D2_SpoofTypeLabel, D2_IlluminationLabel,D2_LiveLabel])
        # model = Model(inputs = input, outputs = FTGenerator )
        return model
...
model = ResnetBuilder.build_resnet_18(input_shape=(3,256,256))
model.summary()
